...
program_version = 'Auto Trade v4.2_2022-02-09'
...
def auto_trade_start():
    ...
    post_message(chat_id,"손익비 : {:0,.2f}".format(FnL_ratio))
    today_list = []
    for t in ticker:
            ...
            for t in ticker:
                ...
                if target_price < current_price and high_price < target_price *1.02 and ma5 < current_price and start_time < now < end_time:
                    if t not in buy_list and krw > 5000:
                        ...
                        post_message(chat_id, "목표가 : {:0,.0f}원".format(target_price))
                        post_message(chat_id, "매수가 : {:0,.0f}원".format(avr_price))
                        post_message(chat_id, "슬리피지 : {:0,.1f}%".format(slip))
